...
2. **`NStepReplayBuffer`** class - N-step replay buffer for improved credit assignment. We keep a snippet of the transition so we can compute the n-step return.
...
- The `gather()` function from Pytorch is essential for selecting Q-values of specific actions from a Q-value table
