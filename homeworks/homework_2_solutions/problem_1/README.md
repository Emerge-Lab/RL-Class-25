...
Here we implement some of the standard machinery used to compute the advantages.
...
We need to be able to compute log probabilities, we'll work that out here.
...
- Start with Part 1 (`compute_returns`).
...
PPO is an **on-policy** algorithm: it collects data with the current policy, uses that data for a few gradient updates, then throws it away and collects fresh data. This means PPO often needs a *lot* of environment interaction relative to the number of gradient steps. With a single environment, most of the wall-clock time is spent waiting for `env.step()` rather than training the network.
...
- `last_value[i]` = V(s) for the state AFTER the last collected step in env i. Some environments auto-reset once the episode ends so we don't have this state; in that case a common approximation is to just reuse the final state before the reset.
...
2. Treat partial episodes as if they ended with 0 future reward which would be fundamentally incorrect.
